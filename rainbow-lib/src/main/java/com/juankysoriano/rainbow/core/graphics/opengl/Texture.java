...
public class Texture {
  ...
  public int glWidth;
  public int glHeight;
  private RainbowGraphicsOpenGL.GLResourceTexture glres;

  protected RainbowGraphicsOpenGL pg;
  ...
  public Texture(RainbowGraphicsOpenGL pg) {
    this.pg = pg;
    rainbowGL = pg.rainbowGl;
   ...
   * @param height  int
   */
  public Texture(RainbowGraphicsOpenGL pg, int width, int height) {
   ...
   */
  public Texture(RainbowGraphicsOpenGL pg, int width, int height, Object params) {
    this.pg = pg;
    rainbowGL = pg.rainbowGl;
    context = rainbowGL.createEmptyContext();
  ...
  public void init(int width, int height,
    ...
    usingMipmaps = glMinFilter == RainbowGL.LINEAR_MIPMAP_NEAREST ||
                   glMinFilter == RainbowGL.LINEAR_MIPMAP_LINEAR;

    usingRepeat = glWrapS == RainbowGL.REPEAT || glWrapT == RainbowGL.REPEAT;
  }
  ...
  public void set(int[] pixels, int x, int y, int w, int h, int format) {
    if (pixels == null) {
      RainbowGraphics.showWarning("The pixels array is null.");
      return;
    }
    if (pixels.length < w * h) {
      RainbowGraphics.showWarning("The pixel array has a length of " +
    ...
    if (!rainbowGL.texturingIsEnabled(glTarget)) {
      rainbowGL.enableTexturing(glTarget);
      enabledTex = true;
    }
    rainbowGL.bindTexture(glTarget, glName);
    ...
    rainbowGL.texSubImage2D(glTarget, 0, x, y, w, h, RainbowGL.RGBA, RainbowGL.UNSIGNED_BYTE,
                            pixelBuffer);
    fillEdges(x, y, w, h);

    if (usingMipmaps) {
      if (RainbowGraphicsOpenGL.autoMipmapGenSupported) {
        rainbowGL.generateMipmap(glTarget);
      } else {
        manualMipmap();
      }
    }

    rainbowGL.bindTexture(glTarget, 0);
    if (enabledTex) {
      rainbowGL.disableTexturing(glTarget);
    }
  ...
  public void setNative(IntBuffer pixBuf, int x, int y, int w, int h) {
    if (pixBuf == null) {
      pixBuf = null;
      RainbowGraphics.showWarning("The pixel buffer is null.");
      return;
    }
    if (pixBuf.capacity() < w * h) {
      RainbowGraphics.showWarning("The pixel bufer has a length of " +
  ...
  public void usingMipmaps(boolean mipmaps, int sampling)  {
    ...
    if (mipmaps) {
      if (sampling == POINT) {
        glMagFilter = RainbowGL.NEAREST;
        glMinFilter = RainbowGL.NEAREST;
        usingMipmaps = false;
      } else if (sampling == LINEAR)  {
        glMagFilter = RainbowGL.NEAREST;
        glMinFilter =
          RainbowGL.MIPMAPS_ENABLED ? RainbowGL.LINEAR_MIPMAP_NEAREST : RainbowGL.LINEAR;
        usingMipmaps = true;
      } else if (sampling == BILINEAR)  {
        glMagFilter = RainbowGL.LINEAR;
        glMinFilter =
          RainbowGL.MIPMAPS_ENABLED ? RainbowGL.LINEAR_MIPMAP_NEAREST : RainbowGL.LINEAR;
        usingMipmaps = true;
        ...
        glMagFilter = RainbowGL.LINEAR;
        glMinFilter =
          RainbowGL.MIPMAPS_ENABLED ? RainbowGL.LINEAR_MIPMAP_LINEAR : RainbowGL.LINEAR;
        usingMipmaps = true;
      } else {
    ...
    if (glMagFilter0 != glMagFilter || glMinFilter0 != glMinFilter) {
      bind();
      rainbowGL.texParameteri(glTarget, RainbowGL.TEXTURE_MIN_FILTER, glMinFilter);
      rainbowGL.texParameteri(glTarget, RainbowGL.TEXTURE_MAG_FILTER, glMagFilter);
      if (usingMipmaps) {
        if (RainbowGraphicsOpenGL.autoMipmapGenSupported) {
          rainbowGL.generateMipmap(glTarget);
        } else {
  ...
  public void usingRepeat(boolean repeat)  {
    if (repeat) {
      glWrapS = RainbowGL.REPEAT;
      glWrapT = RainbowGL.REPEAT;
      usingRepeat = true;
    } else {
      glWrapS = RainbowGL.CLAMP_TO_EDGE;
      glWrapT = RainbowGL.CLAMP_TO_EDGE;
      usingRepeat = false;
    }

    bind();
    rainbowGL.texParameteri(glTarget, RainbowGL.TEXTURE_WRAP_S, glWrapS);
    rainbowGL.texParameteri(glTarget, RainbowGL.TEXTURE_WRAP_T, glWrapT);
    unbind();
  }
  ...
  public int currentSampling() {
    if (glMagFilter == RainbowGL.NEAREST && glMinFilter == RainbowGL.NEAREST) {
      return POINT;
    } else if (glMagFilter == RainbowGL.NEAREST &&
               glMinFilter == (RainbowGL.MIPMAPS_ENABLED ? RainbowGL.LINEAR_MIPMAP_NEAREST : RainbowGL.LINEAR)) {
      return LINEAR;
    } else if (glMagFilter == RainbowGL.LINEAR &&
               glMinFilter == (RainbowGL.MIPMAPS_ENABLED ? RainbowGL.LINEAR_MIPMAP_NEAREST : RainbowGL.LINEAR)) {
      return BILINEAR;
    } else if (glMagFilter == RainbowGL.LINEAR &&
               glMinFilter == RainbowGL.LINEAR_MIPMAP_LINEAR) {
  ...
  public void unbind() {
    if (rainbowGL.textureIsBound(glTarget, glName)) {
  ...
  protected void updateTexelsImpl(int x, int y, int w, int h) {
    ...
    if (!modified) {
      ...
      mx2 = RainbowMath.min(width - 1, x2);
      my1 = RainbowMath.max(0, y);
      my2 = RainbowMath.min(height - 1, y2);
      modified = true;
  ...
  protected void updatePixelBuffer(int[] pixels) {
    pixelBuffer = RainbowGL.updateIntBuffer(pixelBuffer, pixels, true);
    pixBufUpdateCount++;
  }
  ...
  public void disposeSourceBuffer() {
    ...
    while (0 < usedBuffers.size()) {
      ...
      try {
        ...
        RainbowGraphics.showWarning("Cannot remove used buffer");
      }
      if (data != null) {
        data.dispose();
      }
  ...
  protected boolean bufferUpdate() {
    ...
    try {
      ...
      RainbowGraphics.showWarning("Don't have pixel data to copy to texture");
    }
  ...
  protected void setSize(int w, int h) {
    ...
    if (RainbowGraphicsOpenGL.npotTexSupported) {
      glWidth = w;
      glHeight = h;
    } else {
      glWidth = RainbowGL.nextPowerOfTwo(w);
      glHeight = RainbowGL.nextPowerOfTwo(h);
    }

    if (glWidth > RainbowGraphicsOpenGL.maxTextureSize ||
        glHeight > RainbowGraphicsOpenGL.maxTextureSize) {
      glWidth = glHeight = 0;
      throw new RuntimeException("Image width and height cannot be" +
                                 " larger than " +
                                 RainbowGraphicsOpenGL.maxTextureSize +
  ...
  protected void allocate() {
    ...
    glres = new RainbowGraphicsOpenGL.GLResourceTexture(this);
    ...
    rainbowGL.texParameteri(glTarget, RainbowGL.TEXTURE_WRAP_S, glWrapS);
    rainbowGL.texParameteri(glTarget, RainbowGL.TEXTURE_WRAP_T, glWrapT);
    if (RainbowGraphicsOpenGL.anisoSamplingSupported) {
      rainbowGL.texParameterf(glTarget, RainbowGL.TEXTURE_MAX_ANISOTROPY,
                              RainbowGraphicsOpenGL.maxAnisoAmount);
    }
    ...
    // textures)
    rainbowGL.texImage2D(glTarget, 0, glFormat, glWidth, glHeight, 0,
                         RainbowGL.RGBA, RainbowGL.UNSIGNED_BYTE, null);

    // Makes sure that the texture buffer in video memory doesn't contain
    // any garbage.
    rainbowGL.initTexture(glTarget, RainbowGL.RGBA, width, height);
  ...
  protected void copyTexture(Texture tex, int x, int y, int w, int h,
    ...
    rainbowGL.flush(); // Needed to make sure that the change in this texture is
  ...
  protected void copyTexture(int texTarget, int texName,
    ...
    if (scale) {
      // Rendering tex into "this", and scaling the source rectangle
      // to cover the entire destination region.
      rainbowGL.drawTexture(texTarget, texName, texWidth, texHeight,
  ...
  public Parameters getParameters() {
    ...
    if (glTarget == RainbowGL.TEXTURE_2D)  {
      res.target = TEX2D;
    }

    if (glFormat == RainbowGL.RGB)  {
      res.format = RGB;
    } else  if (glFormat == RainbowGL.RGBA) {
      res.format = ARGB;
    } else  if (glFormat == RainbowGL.ALPHA) {
      res.format = ALPHA;
    }
    ...
    if (glMagFilter == RainbowGL.NEAREST && glMinFilter == RainbowGL.NEAREST) {
      res.sampling = POINT;
      res.mipmaps = false;
    } else if (glMagFilter == RainbowGL.NEAREST && glMinFilter == RainbowGL.LINEAR)  {
      res.sampling = LINEAR;
      res.mipmaps = false;
    } else if (glMagFilter == RainbowGL.NEAREST &&
               glMinFilter == RainbowGL.LINEAR_MIPMAP_NEAREST)  {
      res.sampling = LINEAR;
      res.mipmaps = true;
    } else if (glMagFilter == RainbowGL.LINEAR && glMinFilter == RainbowGL.LINEAR)  {
    ...
    if (glWrapS == RainbowGL.CLAMP_TO_EDGE) {
      res.wrapU = CLAMP;
    } else if (glWrapS == RainbowGL.REPEAT) {
      res.wrapU = REPEAT;
    }

    if (glWrapT == RainbowGL.CLAMP_TO_EDGE) {
      res.wrapV = CLAMP;
    } else if (glWrapT == RainbowGL.REPEAT) {
      res.wrapV = REPEAT;
    }
  ...
  protected void setParameters(Parameters params) {
    if (params.target == TEX2D)  {
        glTarget = RainbowGL.TEXTURE_2D;
    } else {
      throw new RuntimeException("Unknown texture target");
    }

    if (params.format == RGB)  {
      glFormat = RainbowGL.RGB;
    } else  if (params.format == ARGB) {
      glFormat = RainbowGL.RGBA;
    } else  if (params.format == ALPHA) {
      glFormat = RainbowGL.ALPHA;
    } else {
      throw new RuntimeException("Unknown texture format");
    }

    boolean mipmaps = params.mipmaps && RainbowGL.MIPMAPS_ENABLED;
    if (mipmaps && !RainbowGraphicsOpenGL.autoMipmapGenSupported) {
      RainbowGraphics.showWarning("Mipmaps were requested but automatic mipmap " +
    ...
    if (params.sampling == POINT) {
      ...
      glMagFilter = RainbowGL.NEAREST;
      glMinFilter = mipmaps ? RainbowGL.LINEAR_MIPMAP_NEAREST : RainbowGL.LINEAR;
    } else if (params.sampling == BILINEAR)  {
      glMagFilter = RainbowGL.LINEAR;
      glMinFilter = mipmaps ? RainbowGL.LINEAR_MIPMAP_NEAREST : RainbowGL.LINEAR;
    } else if (params.sampling == TRILINEAR)  {
      glMagFilter = RainbowGL.LINEAR;
      glMinFilter = mipmaps ? RainbowGL.LINEAR_MIPMAP_LINEAR : RainbowGL.LINEAR;
    } else {
      throw new RuntimeException("Unknown texture filtering mode");
    }
  ...
  protected void fillEdges(int x, int y, int w, int h) {
    if ((width < glWidth || height < glHeight) && (x + w == width || y + h == height)) {
      if (x + w == width) {
        ...
        edgeBuffer = RainbowGL.updateIntBuffer(edgeBuffer, edgePixels, true);
        rainbowGL.texSubImage2D(glTarget, 0, width, y, ew, h, RainbowGL.RGBA,
                                RainbowGL.UNSIGNED_BYTE, edgeBuffer);
      }
      ...
      if (y + h == height) {
        ...
        edgeBuffer = RainbowGL.updateIntBuffer(edgeBuffer, edgePixels, true);
        rainbowGL.texSubImage2D(glTarget, 0, x, height, w, eh, RainbowGL.RGBA,
                                RainbowGL.UNSIGNED_BYTE, edgeBuffer);
      }
      ...
      if (x + w == width && y + h == height) {
        ...
        edgeBuffer = RainbowGL.updateIntBuffer(edgeBuffer, edgePixels, true);
        rainbowGL.texSubImage2D(glTarget, 0, width, height, ew, eh, RainbowGL.RGBA,
                                RainbowGL.UNSIGNED_BYTE, edgeBuffer);
      }
    }
  }
