...
package com.juankysoriano.rainbow.core.graphics.opengl.tess;


public class GLUtessellatorImpl implements RainbowGLUtessellator {
    ...
    private RainbowGLUtessellatorCallback callBegin;
    private RainbowGLUtessellatorCallback callEdgeFlag;
    private RainbowGLUtessellatorCallback callVertex;
    private RainbowGLUtessellatorCallback callEnd;
//    private GLUtessellatorCallback callMesh;
    private RainbowGLUtessellatorCallback callError;
    private RainbowGLUtessellatorCallback callCombine;

    private RainbowGLUtessellatorCallback callBeginData;
    private RainbowGLUtessellatorCallback callEdgeFlagData;
    private RainbowGLUtessellatorCallback callVertexData;
    private RainbowGLUtessellatorCallback callEndData;
//    private GLUtessellatorCallback callMeshData;
    private RainbowGLUtessellatorCallback callErrorData;
    private RainbowGLUtessellatorCallback callCombineData;
...
//    private static final int GLU_TESS_MESH = 100112;    /* void (*)(GLUmesh *mesh)        */
    private static RainbowGLUtessellatorCallback NULL_CB = new RainbowGLUtessellatorCallbackAdapter();
...
//                 MAX(sizeof(GLUvertex),sizeof(GLUface))))
    ...
    private GLUtessellatorImpl() {
        ...
        windingRule = RainbowGLU.GLU_TESS_WINDING_ODD;
        flagBoundary = false;
        boundaryOnly = false;
...
//        callMesh = NULL_CB;
    ...
    static public RainbowGLUtessellator gluNewTess()
    {
        ...
        while (state != newState) {
            ...
            if (state < newState) {
                if (state == TessState.T_DORMANT) {
                    callErrorOrErrorData(RainbowGLU.GLU_TESS_MISSING_BEGIN_POLYGON);
                    gluTessBeginPolygon(null);
                } else if (state == TessState.T_IN_POLYGON) {
                    callErrorOrErrorData(RainbowGLU.GLU_TESS_MISSING_BEGIN_CONTOUR);
                    gluTessBeginContour();
                }
            } else {
                if (state == TessState.T_IN_CONTOUR) {
                    callErrorOrErrorData(RainbowGLU.GLU_TESS_MISSING_END_CONTOUR);
                    gluTessEndContour();
                } else if (state == TessState.T_IN_POLYGON) {
                    callErrorOrErrorData(RainbowGLU.GLU_TESS_MISSING_END_POLYGON);
    ...
    public void gluTessProperty(int which, double value) {
        switch (which) {
            case RainbowGLU.GLU_TESS_TOLERANCE:
                if (value < 0.0 || value > 1.0) break;
                relTolerance = value;
                return;

            case RainbowGLU.GLU_TESS_WINDING_RULE:
                int windingRule = (int) value;
                if (windingRule != value) break;    /* not an integer */

                switch (windingRule) {
                    case RainbowGLU.GLU_TESS_WINDING_ODD:
                    case RainbowGLU.GLU_TESS_WINDING_NONZERO:
                    case RainbowGLU.GLU_TESS_WINDING_POSITIVE:
                    case RainbowGLU.GLU_TESS_WINDING_NEGATIVE:
                    case RainbowGLU.GLU_TESS_WINDING_ABS_GEQ_TWO:
            ...
            case RainbowGLU.GLU_TESS_BOUNDARY_ONLY:
                boundaryOnly = (value != 0);
                return;

            case RainbowGLU.GLU_TESS_AVOID_DEGENERATE_TRIANGLES:
                avoidDegenerateTris = (value != 0);
                return;

            default:
                callErrorOrErrorData(RainbowGLU.GLU_INVALID_ENUM);
                return;
        }
        callErrorOrErrorData(RainbowGLU.GLU_INVALID_VALUE);
    }
...
/* tolerance should be in range [0..1] */
                ...
                break;
            case RainbowGLU.GLU_TESS_WINDING_RULE:
                assert (windingRule == RainbowGLU.GLU_TESS_WINDING_ODD ||
                        windingRule == RainbowGLU.GLU_TESS_WINDING_NONZERO ||
                        windingRule == RainbowGLU.GLU_TESS_WINDING_POSITIVE ||
                        windingRule == RainbowGLU.GLU_TESS_WINDING_NEGATIVE ||
                        windingRule == RainbowGLU.GLU_TESS_WINDING_ABS_GEQ_TWO);
    ...
    public void gluTessCallback(int which, RainbowGLUtessellatorCallback aCallback) {
        switch (which) {
            case RainbowGLU.GLU_TESS_BEGIN:
                callBegin = aCallback == null ? NULL_CB : aCallback;
                return;
            case RainbowGLU.GLU_TESS_BEGIN_DATA:
                callBeginData = aCallback == null ? NULL_CB : aCallback;
                return;
            case RainbowGLU.GLU_TESS_EDGE_FLAG:
 ...
 */
                ...
                return;
            case RainbowGLU.GLU_TESS_EDGE_FLAG_DATA:
 ...
 */
    ...
    public void gluTessVertex(double[] coords, int coords_offset, Object vertexData) {
        ...
        if (flushCacheOnNextVertex) {
            if (!flushCache()) {
                callErrorOrErrorData(RainbowGLU.GLU_OUT_OF_MEMORY);
                return;
            }
            lastEdge = null;
        }
        for (i = 0; i < 3; ++i) {
            ...
            if (x < -RainbowGLU.GLU_TESS_MAX_COORD) {
                x = -RainbowGLU.GLU_TESS_MAX_COORD;
                tooLarge = true;
            }
            if (x > RainbowGLU.GLU_TESS_MAX_COORD) {
                x = RainbowGLU.GLU_TESS_MAX_COORD;
        ...
        if (tooLarge) {
            callErrorOrErrorData(RainbowGLU.GLU_TESS_COORD_TOO_LARGE);
        }
